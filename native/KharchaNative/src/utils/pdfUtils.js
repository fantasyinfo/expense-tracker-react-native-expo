...
const generatePDFHTML = async (entries, options = {}) => {
  ...
  let html = `
    ...
    <body>
      ...
      <div class="summary">
        <div class="summary-card">
          <div class="summary-label">Expense - ${options.paymentLabels?.upi || 'Digital'}</div>
        ...
        <div class="summary-card">
          <div class="summary-label">Income - ${options.paymentLabels?.upi || 'Digital'}</div>
...
export const exportToPDF = async (entries, options = {}) => {
  try {
    ...
    await RNFS.writeFile(fileUri, htmlContent, 'utf8');
